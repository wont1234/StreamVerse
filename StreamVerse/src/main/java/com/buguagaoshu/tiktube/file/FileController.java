...
public class FileController {
    ...
    @GetMapping("/api/upload/{code}/oss/image/{year}/{month}/{day}/{filename:.+}")
    public ResponseEntity<Void> getOSSImage(@PathVariable(value = "code") Integer code,
        ...
        String date = "image/" + year + "/" + month + "/" + day;
    ...
    public ResponseEntity<Void> getOSS(@PathVariable(value = "code") Integer code,
        ...
        if (FileTypeEnum.getFileType(FileTypeEnum.getFileSuffix(filename)).equals(FileTypeEnum.VIDEO)) {
            ...
            if (key == null) {
                log.info("[PlayLimit] file access denied: missing key, filename={}, articleId={}, fileId={}",
                        filename, fileTableEntity.getArticleId(), fileTableEntity.getId());
                return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
            }
            String originalText = AesUtil.getInstance().decrypt(key);
            if (originalText == null) {
                log.info("[PlayLimit] file access denied: key decrypt failed, filename={}, articleId={}, fileId={}",
                        filename, fileTableEntity.getArticleId(), fileTableEntity.getId());
                return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
            }
            ...
            log.info("[PlayLimit] file access(oss) filename={}, userId={}, fileIdInKey={}, dbFileId={}, articleId={}, expire={}",
                    filename, userId, fileId, fileTableEntity.getId(), fileTableEntity.getArticleId(), expire);
            // 先进行过期时间的判断
            ...
            if (articleService.hasThisVideoPlayPower(fileTableEntity, userId, request)) {
                ...
                log.info("[PlayLimit] file access(oss) denied by playPower, filename={}, userId={}, fileId={}, articleId={}",
                        filename, userId, fileId, fileTableEntity.getArticleId());
    ...
    public ResponseEntity getFile(String date, String filename, String key, HttpServletRequest request) {
        try {
            ...
            if (FileTypeEnum.getFileType(FileTypeEnum.getFileSuffix(filename)).equals(FileTypeEnum.VIDEO)) {
                ...
                // 视频文件 key 错误直接返回 null
                if (key == null) {
                    log.info("[PlayLimit] file access denied: missing key, date={}, filename={}, articleId={}, fileId={}",
                            date, filename, fileTableEntity.getArticleId(), fileTableEntity.getId());
                    return null;
                }
                String originalText = AesUtil.getInstance().decrypt(key);
                if (originalText == null) {
                    log.info("[PlayLimit] file access denied: key decrypt failed, date={}, filename={}, articleId={}, fileId={}",
                            date, filename, fileTableEntity.getArticleId(), fileTableEntity.getId());
                    return null;
                }
                // 第一个值是用户ID，如果访问时没有登录，则这个值是 -1
                ...
                Long fileId = Long.parseLong(msg[1]);
                Long expire = Long.parseLong(msg[2]);
                log.info("[PlayLimit] file access(local) date={}, filename={}, userId={}, fileIdInKey={}, dbFileId={}, articleId={}, expire={}",
                        date, filename, userId, fileId, fileTableEntity.getId(), fileTableEntity.getArticleId(), expire);
                // 先进行过期时间的判断
                if (System.currentTimeMillis() >expire) {
                    log.warn("用户 {} 访问文件id为 {} 的的文件时，使用的 key {} 已过期!", msg[0], msg[1], key);
                    return null;
                ...
                if (articleService.hasThisVideoPlayPower(fileTableEntity, userId, request)) {
                    ...
                    log.info("[PlayLimit] file access(local) denied by playPower, date={}, filename={}, userId={}, fileId={}, articleId={}",
                            date, filename, userId, fileId, fileTableEntity.getArticleId());
                    return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
                }
            }
